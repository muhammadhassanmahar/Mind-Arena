...
class ContestService:
    ...
    async def create_contest(contest_data: ContestCreate) -> ContestOut:
        ...
        contest_dict["created_at"] = datetime.utcnow()
    ...
    @staticmethod
    async def get_contest_by_id(contest_id: str) -> ContestOut:
    ...
    @staticmethod
    async def get_all_contests() -> List[ContestOut]:
    ...
    @staticmethod
    async def get_today_contest(current_time: datetime) -> ContestOut | None:
    ...
    async def update_contest(
        ...
        return await ContestService.get_contest_by_id(contest_id)

    @staticmethod
    async def delete_contest(contest_id: str) -> dict:

        result = await CONTEST_COLLECTION.delete_one(
            {"_id": ObjectId(contest_id)}
        )
